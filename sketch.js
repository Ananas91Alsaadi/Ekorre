...
function setup() {
  let canvas = createCanvas(500, 680);
  ...
  for (let i = 0; i < 60; i++) {
    ...
    for (let j in allSpaceBetween) {
      if (randomSpacebetween < allSpaceBetween[j] - 50 || randomSpacebetween > allSpaceBetween[j] + 50) {
        drawBranches++;
      }  
    }
...
function draw() {
      ...
      for (let i in allSpaceBetween) {
        allSpaceBetween[i]=allSpaceBetween[i]-moveMeter*2+cameraCorrecting;
      }
    }
  
  if (squirrelY<height/3){cameraCorrecting=1;}else{cameraCorrecting=0;}
  ...
  if (allFood.length<1) {
    ...
    let onedirtyFood = new dirtyFood(random(width), random(height*2)-height*2);
    alldirtyFood.push(onedirtyFood);
  }
  ...
  if (allTreeSpots.length<90) {
    let oneSpot = new spots(width / 2 - width / 10 / 2, random(height*2)-height*2);
    allTreeSpots.push(oneSpot);
  }
...
goLeft();
  ...
  if (squirrelJump) {
    ...
    moveMeter = -4;
    if (overHalf==false) {    moveMeter = -6;
}
    
    let steps;
    if (overHalf) {steps=125;}else{steps=300;}
...
function squirrel(x, y) {
  ...
  stroke("#e66910");
  //tail
  bezier(x + 20 * (moveRight), y + 15, x + 70 * (moveRight), y, x + (whileJump + 10) * (moveRight), y - 20 + whileJump, x + (whileJump + 40) * (moveRight), y - 50 + whileJump);
  stroke("#e66910");
  fill("#e66910");
  strokeWeight(1);
  //leg
...
class spots {
  ...
  show() {
    ...
    for (let i = 0; i < this.spots; i++) {
      circle(this.x + this.spaceBetweenX[i], this.y + this.spaceBetweenY[i], this.spotSize[i]);
    }
  }
  ...
  showTree() {
    ...
    this.y = this.y -moveMeter*2+cameraCorrecting;}
    for (let i = 0; i < this.spots; i++) {
      rect(this.x + this.spaceBetweenX[i], this.y + this.spaceBetweenY[i], this.treeSpotSizeW[i], this.treeSpotSizeH[i]);
    }
...
class branches {
  ...
  show() {
    ...
    if (this.leftOrRight) {
      ...
      bezier(width / 2 + width / 10 / 2, this.y, width / 2 + width / 10 / 2 + this.x2, this.y - this.y2, width / 2 + width / 10 / 2 + this.x3, this.y + this.y3, width / 2 + width / 10 / 2 + this.x4, this.y - this.y4);

    } else {

      bezier(width / 2 - width / 10 / 2, this.y, width / 2 - width / 10 - this.x2, this.y - this.y2, width / 2 - width / 10 - this.x3, this.y + this.y3, width / 2 - width / 10 - this.x4, this.y - this.y4);
    }
  }
  ...
  jump() {
    if (finishedJump&&hearts>0) { 
      ...
      if (this.leftOrRight) {
        if ((squirrelY + 30) < (this.y + this.thickness/2) && (squirrelY + 30) > (this.y - this.thickness/2) && squirrelX > (width / 2 + width / 10 / 2) && squirrelX < width / 2 + width / 10 / 2 + this.x4) {
          squirrelJump = true;
          jumpSteps = this.y;
          jumpDown = false;
          finishedJump = false;
                if(squirrelY<height/1.35) {overHalf=true;}
          if (this.firstTouch) {scores++;this.firstTouch=false;}

        }
      } else {
        if ((squirrelY + 30) < (this.y + this.thickness/2) && (squirrelY + 30) > (this.y - this.thickness/2) && squirrelX < (width / 2 - width / 10 / 2) && squirrelX > width / 2 - width / 10 - this.x4) {
          squirrelJump = true;
          jumpSteps = this.y;
          jumpDown = false;
          finishedJump = false;
