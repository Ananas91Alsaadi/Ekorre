...
document.getElementById("signUpButton").addEventListener("click", function () {
	...
	gameAuth.createUserWithEmailAndPassword(playerEmail, playerPassword).then(cons=>{
		userIDgame=cons.user.uid;
						gameDb.collection("scoreBoard").doc(cons.user.uid).set({name:playerName,score:"0"}).then(()=>{
...
function writeSB() {
	scoreBoardGame = scoreBoardGame.sort(function compareNumbers(a, b) {return b.score - a.score;});
	let e=0;
	if (scoreBoardGame.length<5) {e=scoreBoardGame.length}else{e=5}
	for (let i=0;i<e;i++) {
